...
class CSSmin {
	...
	public function run( $css = '', $linebreak_pos = false ) {
		...
		if ( false !== $last_newline ) {
			...
			if ( false !== $last_comment_start ) {
				$last_comment_end = strrpos( $css, '*/', $last_comment_start );
				if ( false === $last_comment_end ) {
	...
	private function do_raise_php_limits() {
		...
		foreach ( $php_limits as $name => $suggested ) {
			...
			if ( $current > -1 && ( -1 === $suggested || $current < $suggested ) ) {
				ini_set( $name, $suggested ); // @codingStandardsIgnoreLine.
			}
		}
	}
	...
	private function minify( $css, $linebreak_pos ) {
		...
		// 000.6 to .6, -0.8 to -.8, 0050 to 50, -01.05 to -1.05 @codingStandardsIgnoreLine.
		...
		// Replace 0 0; or 0 0 0; or 0 0 0 0; with 0. @codingStandardsIgnoreLine.
		...
		// Shorten colors from rgb(51,102,153) to #336699, rgb(100%,0%,0%) to #ff0000 (sRGB color space) @codingStandardsIgnoreLine.
	...
	private function extract_data_urls( $css ) {
		...
		while ( preg_match( $pattern, $css, $m, 0, $offset ) ) {
			...
			$start_index = $index + 4; // "url(".length() @codingStandardsIgnoreLine.
	...
	private function preserve_old_IE_specific_matrix_definition( $matches ) { // @codingStandardsIgnoreLine.
	...
	private function rgb_to_hex( $matches ) {
		// Support for percentage values rgb(100%, 0%, 45%); @codingStandardsIgnoreLine.
